...
function calcularOrcamento(qtd, preco, formato, cor, valorChapa, valorCtp, extra) {
    var qtdTotal = calcQtd(qtd, formato, cor)
    var precoFinal = qtdTotal * preco
    if (cor) {
        precoFinal += 4 * valorChapa
        precoFinal = precoFinal * 2.4
        precoFinal += 4 * valorCtp
    } else {
        precoFinal += valorChapa
        precoFinal = precoFinal * 2.1
    }
    if (extra !== null) precoFinal += parseFloat(extra)
    return precoFinal
}